    ...
    def has_played(self,first_player,second_player):
        ...
        for round in self.rounds:
            for match in round.matchs:
                if match.first_player.name == first_player and match.second_player.name == second_player:
                    #print("match déjà joué, joueur 1 " + match.first_player.name + " = " + first_player + "joueur 2 " + match.second_player.name + " = "+ second_player )
                    played = True
                elif match.first_player.name == second_player and match.second_player.name == first_player:
                    #print("match déjà joué, joueur 1 " + match.first_player.name + " = " + first_player + "joueur 2 " + match.second_player.name + " = "+ second_player )
                    played = True
                else:
                    pass
